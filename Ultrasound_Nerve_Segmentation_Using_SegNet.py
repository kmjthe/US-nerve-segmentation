...
def train_generator(data_frame, batch_size, train_path, aug_dict,
    ...
    # print(train_gen)      <zip object at 0x000001D68E42C0C8>
...
for k, (train_index, test_index) in enumerate(kf.split(df)):
    ...
    # type(train_gen)     # <generator object train_generator at 0x00000274F68CF9C8>

    test_gener = train_generator(test_data_frame, BATCH_SIZE,
                                None,
                                dict(),
                                target_size=(height, width))
    # type(test_gener)    # <generator object train_generator at 0x00000274F68CFAC8>

    model = segnet(input_size=(height,width, 1))

    # print(model.summary())
    
    model_checkpoint = ModelCheckpoint(str(k+1) + '_ner_seg.hdf5', verbose=1, save_best_only=True)

    earlystopping = EarlyStopping(monitor='val_loss', patience=15)

    # model.compile(optimizer= SGD(lr=0.01), loss= dice_coef_loss, metrics=[jacard, dice_coef, 'binary_accuracy'])

    # 위치 변경
    model = load_model(str(k+1) + '_ner_seg.hdf5', custom_objects={'dice_coef_loss': dice_coef_loss, 'jacard': jacard, 'dice_coef': dice_coef})

    model.compile(optimizer=Adam(lr=1e-2), loss=dice_coef_loss, metrics=[jacard, dice_coef, 'binary_accuracy'])
    ...
    # model.save_weights(str(EPOCHS) + "_ner_seg.hdf5")
...
model = load_model('1_ner_seg.hdf5', custom_objects={'dice_coef_loss': dice_coef_loss, 'jacard': jacard, 'dice_coef': dice_coef})
# model = load_model('1_ner_seg.hdf5', custom_objects={'dice_coef_loss': dice_coef_loss, 'jacard': jacard, 'dice_coef': dice_coef})
